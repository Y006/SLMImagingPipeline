...
class VirtualSensor:
    """
    临时占位 VirtualSensor 类，用于让 Mask.from_sensor() 可以正常运行。
    未来可以替换为真实实现。
    """

    def __init__(self, resolution=(512, 512), size=(6.4e-3, 4.8e-3), pixel_size=(1.25e-5, 1.25e-5)):
        self.resolution = np.array(resolution)
        self.size = np.array(size)
        self.pixel_size = np.array(pixel_size)

    @classmethod
    def from_name(cls, sensor_name="DefaultSensor", downsample=None):
        """
        模拟从传感器名称创建虚拟传感器。
        参数:
            sensor_name (str): 传感器名称 (占位)
            downsample (float): 下采样比例，可选
        """
        base_resolution = np.array([1024, 1024])
        base_size = np.array([6.4e-3, 4.8e-3])  # 6.4mm × 4.8mm
        pixel_size = base_size / base_resolution

        if downsample is not None and downsample > 1:
            base_resolution = (base_resolution / downsample).astype(int)
            pixel_size = base_size / base_resolution
...
def resize(img, factor=None, shape=None, interpolation=cv.INTER_CUBIC):
    """
    ...
    if torch_available:
        ...
        resized = np.array(
            [
                cv.resize(img[i], dsize=tuple(new_shape[::-1]), interpolation=interpolation)
...
class RandomBinaryMask(Mask):
    """
    Random Binary Mask
    ==================

    Generates a random binary mask (0 or 1 values), 
    similar to a random speckle pattern.

    Example
    -------
    .. code-block:: python

        mask = RandomBinaryMask(resolution=(512,512), feature_size=2.2e-6)
        mask.plot()
    """

    def __init__(self, fill_ratio=0.5, seed=None, **kwargs):
        """
        Random binary mask constructor.

        Parameters
        ----------
        fill_ratio : float, optional
            Fraction of pixels set to 1 (white). Default is 0.5 (i.e. 50% ones, 50% zeros).
        seed : int, optional
            Random seed for reproducibility.
        **kwargs :
            Additional parameters passed to parent Mask class.
        """
    ...
    def create_mask(self):
        """
        Create the random binary mask.
        """
        if self.seed is not None:
            np.random.seed(self.seed)

        # Random uniform matrix [0,1)
        rand_matrix = np.random.rand(self.resolution[0], self.resolution[1])

        # Convert to binary according to fill_ratio
        self.mask = (rand_matrix < self.fill_ratio).astype(np.uint8)