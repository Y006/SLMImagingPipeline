...
def run_measurement_logic_new(config_path: str, repeat: int, note: str):
    """
    Measurement 采集逻辑 (ScreenPro 版本)
    ...
    try:
        # --- 2. 启动显示器 ---
        # 优先使用 screen_pro 配置，否则回退到旧的 display_image 方式
        screen_pro_config = raw_cap_conf.get("screen_pro")
        ...
        if screen_pro_config:
            ...
            logger.info(f"正在显示图片: {disp_path.name} (显示器 {mon_idx}) [旧方式]")
            t = threading.Thread(
                target=display_image,
                args=(str(disp_path), mon_idx, scale_f),
                daemon=True
...
def run_calibration_logic_new(config_path: str):
    """
    标定/预览模式 (ScreenPro 版本)
    
    流程：加载配置 -> 启动显示器 -> 启动 SLM -> 挂起等待
    特点：完全没有相机代码，支持 ScreenPro
